...
def salvar_ranking(nome, pontosjg):
    try:
        with open('saves/points/ranking5.json', 'r') as f:
            ranking = json.load(f)
    ...
    with open('saves/points/ranking5.json', 'w') as f:
        json.dump(ranking, f)
...
def exibir_ranking(tela, pontosjg):
    ...
    sombra_titulo = font_titulo.render("Ranking Nível 5", True, (50, 50, 50))  # Sombra cinza
    tela.blit(sombra_titulo, (tela.get_width() // 2 - sombra_titulo.get_width() // 2 + 5, 130))  # Deslocado

    # Título principal em amarelo
    texto_titulo = font_titulo.render("Ranking Nível 5", True, (255, 255, 0))  # Amarelo
...
def nivel5():
    ...
    pygame.display.set_caption("Nível 5")
    ...
    perguntas = [
        {
            "pergunta": "Características de um algoritmo determinístico:",
            "opcoes": ['Mesmo resultado para a mesma entrada', 'Sempre mais rápido', 'Não usa memória'],
            "correta": 0
        },
        {
            "pergunta": "Exemplo de entrada e saída em um algoritmo:",
            "opcoes": ['Ler de um arquivo e escrever em outro', 'Apenas ler de um banco de dados', 'Apenas escrever na tela'],
            "correta": 0
        },
        {
            "pergunta": "O que é um algoritmo de ordenação por seleção?",
            "opcoes": ['Seleciona seguindo um parâmetro', 'Usa recusão', 'Busca em profundidade'],
            "correta": 0
        },
        {
            "pergunta": "O que é um algoritmo de aproximação?",
            "opcoes": ['Encontra soluções próximas do ideal', 'Funciona só com dados exatos', 'Ignora dados'],
            "correta": 0
        },
        {
            "pergunta": "Principal uso de algoritmos em inteligência artificial:",
            "opcoes": ['Tomar decisões baseadas em dados', 'Criar gráficos', 'Aumentar memória do computador'],
            "correta": 0
        },
        {
            "pergunta": "O que é algoritmo de divisão e conquista?",
            "opcoes": ['Divide um problema em problemas menores', 'Aumenta a complexidade', 'Tipo de loop'],
            "correta": 0
        }
    ]
    ...
    while pergunta_atual < len(perguntas):
        ...
        rect_pergunta = pygame.Rect(185, 130, 1000, 120) # Ratângulo Vermelho
        pygame.draw.rect(tela, (255, 0, 0), rect_pergunta)
