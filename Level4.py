...
def salvar_ranking(nome, pontosjg):
    try:
        with open('saves/points/ranking4.json', 'r') as f:
            ranking = json.load(f)
    ...
    with open('saves/points/ranking4.json', 'w') as f:
        json.dump(ranking, f)
...
def exibir_ranking(tela, pontosjg):
    ...
    sombra_titulo = font_titulo.render("Ranking Nível 4", True, (50, 50, 50))  # Sombra cinza
    tela.blit(sombra_titulo, (tela.get_width() // 2 - sombra_titulo.get_width() // 2 + 5, 130))  # Deslocado

    # Título principal em amarelo
    texto_titulo = font_titulo.render("Ranking Nível 4", True, (255, 255, 0))  # Amarelo
...
def nivel4():
    ...
    pygame.display.set_caption("Nível 4")
    ...
    perguntas = [
        {
            "pergunta": "Principal aplicação de algoritmos de otimização:",
            "opcoes": ['Tornar mais eficiente', 'Ordenação simples', 'Pesquisas em banco de dados'],
            "correta": 0
        },
        {
            "pergunta": "Exemplo de saída de um algoritmo:",
            "opcoes": ['Resultado final', 'Diagrama', 'Documentação técnica'],
            "correta": 0
        },
        {
            "pergunta": "Objetivo de um algoritmo de otimização:",
            "opcoes": ['Encontrar a melhor solução', 'Classificar dados aleatoriamente', 'Ignorar restrições'],
            "correta": 0
        },
        {
            "pergunta": "O que é um algoritmo de força bruta?",
            "opcoes": ['Tenta todas as combinações', 'Algoritmo muito rápido', 'Algoritmo de busca'],
            "correta": 0
        },
        {
            "pergunta": "O que é análise de algoritmos?",
            "opcoes": ['Estudo da eficiência', 'Busca em branco de dados', 'Criação de novos algoritmos'],
            "correta": 0
        },
        {
            "pergunta": "Desvantagem de algoritmos de força bruta:",
            "opcoes": ['Ineficientes em tempo', 'Sempre mais rápidos', 'Difíceis de usar'],
            "correta": 0
        }
    ]
