...
def salvar_ranking(nome, pontosjg):
    try:
        with open('saves/points/ranking3.json', 'r') as f:
            ranking = json.load(f)
    ...
    with open('saves/points/ranking3.json', 'w') as f:
        json.dump(ranking, f)
...
def exibir_ranking(tela, pontosjg):
    ...
    sombra_titulo = font_titulo.render("Ranking Nível 3", True, (50, 50, 50))  # Sombra cinza
    tela.blit(sombra_titulo, (tela.get_width() // 2 - sombra_titulo.get_width() // 2 + 5, 130))  # Deslocado

    # Título principal em amarelo
    texto_titulo = font_titulo.render("Ranking Nível 3", True, (255, 255, 0))  # Amarelo
...
def nivel3():
    ...
    perguntas = [
        {
            "pergunta": "O que caracteriza um algoritmo aleatório?",
            "opcoes": ['Produz resultados diferentes', 'Sempre o mesmo resultado', 'É sempre mais rápido'],
            "correta": 0
        },
        {
            "pergunta": "Principal função de um algoritmo:",
            "opcoes": ['Executar tarefas eficientemente', 'Criar gráficos', 'Resolver problemas juntos'],
            "correta": 0
        },
        {
            "pergunta": "Função de uma ordenação:",
            "opcoes": ['Organizar em sequência', 'Criar ordens aleatórias', 'Reduzir informações'],
            "correta": 0
        },
        {
            "pergunta": "O que caracteriza um algoritmo eficiente?",
            "opcoes": ['Menor tempo para executar', 'Processos longos e baratos', 'Mais possibilidades'],
            "correta": 0
        },
        {
            "pergunta": "O que significa algoritmo?",
            "opcoes": ['Método passo a passo', 'Tipo de programa de TV', 'Linguagem de programação'],
            "correta": 0
        },
        {
            "pergunta": "O que é um diagrama de fluxo?",
            "opcoes": ['Representação gráfica de um algoritmo', 'Tipo de código', 'Banco de dados'],
            "correta": 0
        }
    ]
    ...
    while pergunta_atual < len(perguntas):
        ...
        rect_pergunta = pygame.Rect(185, 130, 1000, 120) # Ratângulo Amarelo
        pygame.draw.rect(tela, (255, 211, 0), rect_pergunta)
