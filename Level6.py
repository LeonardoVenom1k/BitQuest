...
def salvar_ranking(nome, pontosjg):
    try:
        with open('saves/points/ranking6.json', 'r') as f:
            ranking = json.load(f)
    ...
    with open('saves/points/ranking6.json', 'w') as f:
        json.dump(ranking, f)
...
def exibir_ranking(tela, pontosjg):
    ...
    sombra_titulo = font_titulo.render("Ranking Nível 6", True, (50, 50, 50))  # Sombra cinza
    tela.blit(sombra_titulo, (tela.get_width() // 2 - sombra_titulo.get_width() // 2 + 5, 130))  # Deslocado

    # Título principal em amarelo
    texto_titulo = font_titulo.render("Ranking Nível 6", True, (255, 255, 0))  # Amarelo
...
def nivel6():
    ...
    pygame.display.set_caption("Nível 6")
    ...
    perguntas = [
        {
            "pergunta": "O que é programação dinâmica?",
            "opcoes": ['Técnica de otimização que resolve subproblemas', 'Algoritmo de ordenção', 'Método de codificação'],
            "correta": 0
        },
        {
            "pergunta": "O que é um pseudocódigo?",
            "opcoes": ['Representação textual de um algoritmo', 'Tipo de código-fonte', 'Arquivo executável'],
            "correta": 0
        },
        {
            "pergunta": "Resultado de um algoritmo de ordenação estável:",
            "opcoes": ['Elementos iguais mantêm sua ordem', 'Elementos são misturados', 'Não há elementos repetidos'],
            "correta": 0
        },
        {
            "pergunta": "Diferença entre busca em profundidade e largura:",
            "opcoes": ['Profundidade explora um caminho até o final antes de voltar', 'Largura vai mais fundo antes de mudar de nível', 'Ambas seguem o mesmo método'],
            "correta": 0
        },
        {
            "pergunta": "Exemplo de algoritmo que usa divisão e conquista:",
            "opcoes": ['Merge Sort', 'Quick Find', 'Linear Search'],
            "correta": 0
        },
        {
            "pergunta": "Principal característica de um algoritmo recursivo:",
            "opcoes": ['Chamadas repetidas', 'Complexidade fixa', 'Sempre termina rápido'],
            "correta": 0
        }
    ]
