...
def salvar_ranking(nome, pontosjg):
    try:
        with open('saves/points/ranking.json', 'r') as f:
            ranking = json.load(f)
    ...
    with open('saves/points/ranking.json', 'w') as f:
        json.dump(ranking, f)
...
def exibir_ranking(tela, pontosjg):
    ...
    sombra_titulo = font_titulo.render("Ranking Nível 1", True, (50, 50, 50))  # Sombra cinza
    tela.blit(sombra_titulo, (tela.get_width() // 2 - sombra_titulo.get_width() // 2 + 5, 130))  # Deslocado

    # Título principal em amarelo
    texto_titulo = font_titulo.render("Ranking Nível 1", True, (255, 255, 0))  # Amarelo
...
def nivel1():
    ...
    perguntas = [
        {
            "pergunta": "Qual é uma propriedade de um passo a passo?",
            "opcoes": ['Deve ser claro', 'Deve ser longo', 'Deve deixar espaço para interpretações'],
            "correta": 0
        },
        {
            "pergunta": "Exemplo de uso de algoritmos no dia a dia:",
            "opcoes": ['Instruções de receita', 'Apenas cálculos', 'Design de sites'],
            "correta": 0
        },
        {
            "pergunta": "O que significa eficiência?",
            "opcoes": ['Fazer o trabalho com menos tempo e recursos', 'Ter processos longos', 'Ter muitos passos'],
            "correta": 0
        },
        {
            "pergunta": "O que é um algoritmo?",
            "opcoes": ['Sequência de instruções', 'Tipo de software', 'Hardware de computador'],
            "correta": 0
        },
        {
            "pergunta": "O que é uma variável?",
            "opcoes": ['Algo que pode mudar', 'Tipo de algoritmo', 'Função matemática'],
            "correta": 0
        },
        {
            "pergunta": "Importância de um algoritmo:",
            "opcoes": ['Facilitar a resolução de problemas', 'Aumentar custos', 'Tornar processos mais longos'],
            "correta": 0
        }
    ]
    ...
    while pergunta_atual < len(perguntas):
        ...
        rect_pergunta = pygame.Rect(185, 130, 1000, 120) # Retângulo Verde
